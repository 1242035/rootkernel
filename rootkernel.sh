...
add_vendor_overlay() {
	...
	add_file_context "/vendor(.*)" "system_file" "/vendor overlay"
	
	perform mkdir $RAMDISK/$VENDOR_OVL
	perform mkdir $RAMDISK/$VENDOR_OVL/lib
	[ $PLATFORM -eq 64 ] && perform mkdir $RAMDISK/$VENDOR_OVL/lib64
	...
	perform echo -e >>$initrc "    restorecon_recursive /vendor"
...
add_drmfix() {
	...
	[ $PLATFORM -eq 64 ] && perform cp -a $TOOLS/libdrmfix64.so $RAMDISK/$VENDOR_OVL/lib64/libdrmfix.so@0644
